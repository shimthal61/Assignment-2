...
We can see that there is an effect in our model - the *p*-value is pretty small, but we don't yet know what direction this is in. Although we could refer to the condition means, I'm going to run a pairwise comparison using the `emmeans()` function. There is not need to change the adjustment, as we are not running multiple comparisons.
...
First, we should have a read over some of the characteristics of question 2:
...
In this visualisation, I have added in a regression line that depicts the correlation between our covariant, Caffeine, and our DV, Response Time using the `geom_smooth()` function. I was able to remove our experimental factor, Visual Quality, from the regression line by specifying what data to include in the `aes()` function, and for this data to supersede our graph aesthetics using `inherit.aes = FALSE`.

We can see from our regression line that there isn't a particularly strong relationship between Caffeine and Response Time. 
