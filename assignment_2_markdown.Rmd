...
Let's generate some summary statistics and arrange the data from the fastest mean reaction time to the slowest. We need to specify our two grouping variables in the `group()` function call. You'll see later why I've mapped the code chunk to a new variable I'm calling `descriptive_stats`.
...
labels <- descriptive_stats %>%
  filter(Prime == "Positive") %>% 
  mutate(label = case_when(Target == "Negative" ~ "Negative Target",
                           Target == "Positive" ~ "Positive Target"))
head(labels)
```
...
```{r}
descriptive_stats %>%
  ggplot(aes(x = Prime, y = mean_RT)) + 
  geom_line(size = 1.2, aes(group = Target, colour = Target)) +
  geom_point(size = 2.6, aes(colour = Target), shape = 15) +
...
The syntax is very similar to our previous ANOVAS, although there are now two experimental factors which we need to appropriately specify using `aov_4()`. We'll write the term 'Prime \* Target' which corresponds to our two main effects, plus the interaction effect.

The syntax is very similar to what we ran previously, although this time we need to specify our two experimental factors as, using the term Prime \* Target. This term corresponds to a main effect of Prime image, and a main effect of target image, plus the interaction between the two factors. Our factors are both repeated measures factors, we also have to specify them in our random effect term in the same way.

```{r}
factorial_anova <- aov_4(RT ~ Prime * Target + (1 + Target * Prime | participant), data = q3_data_tidied)
...
```{r}
emmeans(factorial_anova, pairwise ~ Prime * Target, adjust = "none")
```

In this output, we are interested in comparing Reaction Times to Negative Targets preceded by Negative vs. Positive Primes. We are also interested in comparing Reaction Times to Positive Targets preceded by Negative vs. Positive Primes. Therefore, the key comparisons are:

-   `Negative Negative - Positive Negative`
-   `Negative Positive - Positive Positive`

Now that we know our meaningful comparisons, we can manually perform a Bonferroni correction test to correct for multiple comparisons. To do this, we multiple the corresponding *p*-values by 2 (which is the number of meaningful comparisons we have). We also need to put a maximum limit of 1 on our *p*-value, as *p* can never exceed 1. 
...
The interaction was interpreted by conducting Bonferroni-corrected pairwise companions. These comparisons revealed that the interaction was driven by Negative Targets being processed faster when preceded by Negative vs. Positive Primes (1,574 ms. vs. 1,567 ms., *t*(292) = 3.08, p = .0064). The interaction was also driven by Positive Targets being processed faster when preceded by Negative vs. Positive Primes (1,563 ms. vs. 1,547 ms., *t*(292) = 2.908, *p* = .008).