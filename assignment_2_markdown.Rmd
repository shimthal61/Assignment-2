...
Now we can add the geom_text to our interaction plot and pass our dataset `labels` to the geom. Among the aesthetics, `geom_text()` includes the aesthetic label, which stands for the text we will add to the visualization. We've also scaled down the plot in the `scale_y_continuous()` function.
...
We can see here that we have a crossover interaction, where we are unlikely to have a main effect of Prime Valence or Target Valence. However, the difference between the